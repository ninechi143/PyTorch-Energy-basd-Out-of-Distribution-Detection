...
class downstream_task_dataset(base_dataset):
    def __init__(self, train_stage, transform, OOD):
        ...
        if OOD:
            indexes = np.array(cifar10.targets) >= 5
        else:
            indexes = np.array(cifar10.targets) < 5


        self.data = np.array(cifar10.data)[indexes]
        self.targets = np.array(cifar10.targets)[indexes]
